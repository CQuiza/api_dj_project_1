...
def login(request):
    ...
    if user is not None:
        ...
        return Response({'token': token.key, 'isSucces': True,'user': serializer.data, 'message': 'Acces granted'}, status=status.HTTP_200_OK)
    
    return Response({'token': 'No token generate', 'isSucces': False, 'user': {}, 'message': 'Invalid credentials'}, status=status.HTTP_400_BAD_REQUEST)
...
def register(request):
    ...
    if serializer.is_valid():
        ...
        token, created = Token.objects.get_or_create(user=user)
        return Response({'token': token.key, 'isSucces': True, 'user': serializer.data, 'message': 'Register granted'}, status=status.HTTP_201_CREATED)
...
def validatorToken(request):
    ...
    if user:
        ...
        return Response({'token':token.key, 'isSucces': True, 'user': serializer.data,'message': 'Token is valid'}, status=status.HTTP_200_OK)
    else:
        return Response({'error': 'Token not found or invalid credencials'}, status=status.HTTP_400_BAD_REQUEST)
